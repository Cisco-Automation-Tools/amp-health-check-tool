...
def save_namurls():
    ...
    for item in NAMADDRESSLIST:
        nam_urls.append(item)

    results["nam_urls"] = nam_urls
    ...
    try:
        home = str(Path.home())
        print(home)
        newpath = os.path.join(home, "nam_urls.json")
        with open(newpath, 'w') as fp:
            fp.write(json_formatted_str)
        fp.close()
    except Exception as e:
        logging.error("%s: Error writing NAMADDRESSLIST nam_urls.json", e)
    print(json_formatted_str)
...
def loadNamUrls():
    try:
        ...
        try:
            data = json.load(fp)
            for item in data["nam_urls"]:
                NAMADDRESSLIST.append(item)

            print(data["resources"])
            pol = data["resources"]

            RESOURCES["isolation_code"] = pol["isolation_code"]
            RESOURCES["policy_serial_compare"] = pol["policy_serial_compare"]
            RESOURCES["tetra_def_compare_64"] = pol["tetra_def_compare_64"]
            RESOURCES["tetra_def_compare_32"] = pol["tetra_def_compare_32"]
            RESOURCES["verify_api_creds"] = pol["verify_api_creds"]
        ...
        logging.error("Error opening file %s", newpath)
...
def main():
    ...
    layout = [
            ...
            Credentials are valid. Can read from text file named "apiCreds.txt" in the local \
                directory.\nMust be in this format:\nclient_id="abcdabcdabcdabcdabcd"\napi_key= \
                    "abcd1234-abcd-1234-abcd-abcd1234abcd"'), sg.Text("", size=(6, 1), \
                                                                      key="_api_cred_valid"),
         sg.Button("Check API Credentials", button_color=('black', '#F0F0F0'), size=button_size, \
        ...
        [sg.Button("Generate Diagnostic", button_color=('black', '#F0F0F0'), size=button_size, \
         ...
         sg.Button("Settings", button_color=('black', '#F0F0F0'), size=button_size, \
        ...
        [sg.Text('Log Level: ', tooltip="Select higher log level if requested by the \
            tool developers."), sg.Button('INFO', button_color=('white', 'green'), \
                                          key='_INFO'), sg.Button('WARNING', button_color=('black', '#F0F0F0'), \
                                                                  key="_WARNING"),
         sg.Button('DEBUG', button_color=('black', '#F0F0F0'), \
                   key="_DEBUG")],
        [sg.Text('', size=(8, 1))],
        [sg.Text('', size=(13, 1)), sg.Button("Refresh", size=(7, 1), button_color=('black', \
                                                                                    '#F0F0F0'),
    ...
    while True:
        ...
        if event in (None, "Cancel"):
            ...
            window.Refresh()
        elif event == "Check API Credentials":
